<%@ page import="model.Company_test" %>
...
//    Company_test company = (Company_test) session.getAttribute("company");
//    ArrayList<Company_test> list = new ArrayList<>();
//    list.add(company);
...
<table border="1">
    <c:forEach items="<%=maping%>" var="map">
        <%--&lt;%&ndash;<a>${map.key}</a><br>&ndash;%&gt;--%>
        <%--<a>${map.value.cash}</a><br>--%>
        <%--<a>${map.value.nonCurrentAssets}</a><br>--%>
        <%--<a>${map.value.longTermLiabilities}</a><br>--%>
        <tr>
            <td>Денежные средства1</td>
            <td>${map.value.cash}</td>
        </tr>
        <tr>
            <td>Денежные средства2</td>
            <td>${map.value.nonCurrentAssets}</td>
        </tr>
        <tr>
            <td>Денежные средства3</td>
            <td>${map.value.longTermLiabilities}</td>
        </tr>
    </c:forEach>
</table>
...
<%--<th><p>ROE</p></th>--%>
<%--</tr>--%>
<%--&lt;%&ndash;<c:forEach items="${listDB}" var="list">&ndash;%&gt;--%>
<%--<c:forEach items="<%=list%>" var="list">--%>
<%--<tr>--%>
<%--<td>${list.name}</td>--%>
<%--<td>${list.tiker}</td>--%>
<%--<td>${list.market_price} RUB</td>--%>
<%--<td>${list.p_e}</td>--%>
<%--<td>${list.p_s}</td>--%>
<%--<td>${list.p_bv}</td>--%>
<%--<td>${list.ev_ebitda}</td>--%>
<%--<td>${list.ev_s}</td>--%>
<%--<td>${list.debit_ebita}</td>--%>
<%--<td>${list.roe}%</td>--%>
<%--</tr>--%>
<%--</c:forEach>--%>
...
<table border="1">
    ...
    <c:forEach var="list" items="<%=listData%>">
        ...
        <tr>
            ...
            <td>${list.currentAssets}</td>
        </tr>
        <tr>
            <td>Внеоборотные активы</td>
            <td>${list.nonCurrentAssets}</td>
        </tr>
        <tr>
            <td>Итого активы</td>
            <td>${list.totalAssets}</td>
        </tr>
        <tr>
            <td>Краткосрочные обстоятельства</td>
            <td>${list.shortTermLiabilities}</td>
        </tr>
        <tr>
            <td>Долгосрочные обстоятельства</td>
            <td>${list.longTermLiabilities}</td>
        </tr>
        <tr>
            <td>Итого обязательства</td>
            <td>${list.totalLiabilities}</td>
        </tr>
        <tr>
            <td>Итого каапитал</td>
            <td>${list.totalCapital}</td>
        </tr>
    </c:forEach>
</table>
<br>
<table border="1">
    ...
    <c:forEach var="list" items="<%=listFinance%>">
        <tr>
            <td>Выручка</td>
            <td>${list.revenue}</td>
        </tr>
        <tr>
            ...
            <td>${list.operatingProfit}</td>
        </tr>
        <tr>
            <td>Прибыль до налогов</td>
            <td>${list.proofitBeforTax}</td>
        </tr>
        <tr>
            <td>Чистая прибыль</td>
            <td>${list.clearnProfit}</td>
        </tr>
        <tr>
            <td>Финансовые доходы</td>
            <td>${list.financealIncome}</td>
        </tr>
        <tr>
            <td>Финансовые расходы</td>
            <td>${list.financealExpenses}</td>
        </tr>
        <tr>
            <td>Амортизация</td>
            <td>${list.depreciation}</td>
        </tr>
    </c:forEach>
...
<table border="1">
    ...
    <c:forEach var="list" items="<%=listMarket%>">
        <tr>
            <td>Количество АО</td>
            <td>${list.numberAO}</td>
        </tr>
